...
public sealed class PatternRenderer
{
    ...
    /// <summary>
    /// Renders multiple target characters into one composed bitmap of square blocks.
    /// </summary>
    /// <param name="settings">Render settings.</param>
    /// <param name="targetCharacters">Ordered character list to render as individual blocks.</param>
    /// <returns>A render result containing the composed output bitmap.</returns>
    public RenderResult RenderCharacterBlocks(RenderSettings settings, IReadOnlyList<string> targetCharacters)
    {
        ArgumentNullException.ThrowIfNull(settings);
        ArgumentNullException.ThrowIfNull(targetCharacters);

        if (targetCharacters.Count == 0)
        {
            throw new ArgumentException("At least one target character is required.", nameof(targetCharacters));
        }

        int cellSize = Math.Max(1, settings.CellSizePx);
        int gridCount = Math.Max(1, settings.GridCount);
        int squareSize = gridCount * cellSize;
        int spacing = Math.Max(0, settings.BlockSpacingPx);

        int columns = settings.BlockColumns <= 0
            ? targetCharacters.Count
            : Math.Clamp(settings.BlockColumns, 1, targetCharacters.Count);

        int rows = (targetCharacters.Count + columns - 1) / columns;
        int width = (columns * squareSize) + ((columns - 1) * spacing);
        int height = (rows * squareSize) + ((rows - 1) * spacing);
        ...
        try
        {
            ...
            for (int i = 0; i < targetCharacters.Count; i++)
            {
                ...
                int y = row * (squareSize + spacing);

                RenderResult blockResult = Render(settings, targetCharacters[i]);
                try
                {
                    g.DrawImage(blockResult.OutputBitmap, x, y, squareSize, squareSize);
                }
                finally
                {
                    blockResult.MaskBitmap?.Dispose();
                    blockResult.OutputBitmap.Dispose();
                }
            }
